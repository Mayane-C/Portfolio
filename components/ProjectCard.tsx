...
export function ProjectCard({ project }: ProjectCardProps) {
  ...
  return (
    ...
    >
      ...
      <div
        aria-hidden
        className="pointer-events-none absolute right-0 top-2 font-serif italic text-[8rem] leading-none text-rose-ancien/8 transition-colors group-hover:text-rose-ancien/15 md:text-[10rem]"
      >
        {project.nr}
      </div>
      ...
      {/* Sous-titre serif italique */}
      <p className="mt-5 max-w-[28rem] font-serif italic text-lg text-taupe leading-snug md:text-xl">
        {project.subtitle}
      </p>

      {/* Métadonnée mono — bas de card */}
      <p className="label-mono mt-8 text-platinum">
        {project.meta}
      </p>

      {/* CTA discret en hover */}
      <p className="label-mono mt-3 text-rose-ancien opacity-0 transition-opacity group-hover:opacity-100">
        Voir le projet →
      </p>
    </Link>
  );
